...
namespace OptimaJet.Workflow.MySQL
{
    public class MySQLProvider : IWorkflowProvider, IMigratable
    {
        ...
        public MySQLProvider(string connectionString, bool writeToHistory = true, bool writeSubProcessToRoot = false) 
            : this(new PersistenceProviderOptions(connectionString)
        {
            SchemaName = null, WriteToHistory = writeToHistory, WriteSubProcessToRoot = writeSubProcessToRoot
        }) {}
        ...
        public MySQLProvider(PersistenceProviderOptions options)
        {
            ...
            //Creating DbObjects
            WorkflowProcessInstance = new WorkflowProcessInstance(Options.GlobalCommandTimeout);
            WorkflowProcessInstanceStatus = new WorkflowProcessInstanceStatus(Options.GlobalCommandTimeout);
            WorkflowProcessInstancePersistence = new WorkflowProcessInstancePersistence(Options.GlobalCommandTimeout);
            WorkflowProcessTransitionHistory = new WorkflowProcessTransitionHistory(Options.GlobalCommandTimeout);
            WorkflowProcessTimer = new WorkflowProcessTimer(Options.GlobalCommandTimeout);
            WorkflowInbox = new WorkflowInbox(Options.GlobalCommandTimeout);
            WorkflowApprovalHistory = new WorkflowApprovalHistory(Options.GlobalCommandTimeout);
            WorkflowGlobalParameter = new WorkflowGlobalParameter(Options.GlobalCommandTimeout);
            WorkflowProcessScheme = new WorkflowProcessScheme(Options.GlobalCommandTimeout);
            WorkflowRuntime = new Models.WorkflowRuntime(Options.GlobalCommandTimeout);
            WorkflowScheme = new WorkflowScheme(Options.GlobalCommandTimeout);
            WorkflowSync = new WorkflowSync(Options.GlobalCommandTimeout);
            ProcessInstanceTree = new ProcessInstanceTree(Options.GlobalCommandTimeout);
            WorkflowForm = new Models.WorkflowForm(Options.GlobalCommandTimeout);
        }
        ...
        private async Task SaveParameterBatchAsync(PersistenceParametersBatch batch)
        {
            ...
            {
                try { await transaction.RollbackAsync().ConfigureAwait(false); } catch { /* ignore rollback errors */ }
                throw;
            }
        }
        ...
        public virtual async Task UpdatePersistenceStateAsync(ProcessInstance processInstance, TransitionDefinition transition)
        {
            ...
            ProcessInstanceEntity inst = await WorkflowProcessInstance.SelectByKeyAsync(connection, processInstance.ProcessId)
                .ConfigureAwait(false);
        ...
        public virtual async Task<ProcessStatus> GetInstanceStatusAsync(Guid processId)
        {
            ...
            ProcessInstanceStatusEntity instance = await WorkflowProcessInstanceStatus.SelectByKeyAsync(connection, processId)
                .ConfigureAwait(false);
        ...
        private async Task SetRunningStatusAsync(Guid processId)
        {
            ...
            ProcessInstanceStatusEntity instanceStatus = await WorkflowProcessInstanceStatus.SelectByKeyAsync(connection, processId)
                .ConfigureAwait(false);
            ...
            ProcessDefinition processDefinition)
        {
            ...
            using (var connection = OpenConnection())
            {
                persistedParameters = (await WorkflowProcessInstancePersistence.SelectByProcessIdAsync(connection,
                        processId)
            ...
            List<ParameterDefinition> persistenceParameters, ProcessInstancePersistenceEntity persistedParameter)
        {
            ...
            else
            {
                result = ParameterDefinition
                    .CreateFromPersistence(parameterDefinition, ParametersSerializer.Deserialize(persistedParameter.Value, parameterDefinition.Type));
            }
        ...
        private async Task SaveGlobalParameterAsync<T>(TenantGlobalParameterKey key, T value)
        {
            ...
            var parameter = (await WorkflowGlobalParameter.SelectByTypeAndNameAsync(connection, key.Type, key.Name, tenantId: key.TenantId)
        ...
        public virtual async Task<DateTime?> GetNextTimerDateAsync(TimerCategory timerCategory, int timerInterval)
        {
            ...
            DateTime? max = await WorkflowRuntime.GetMaxNextTimeAsync(connection, _runtime.Id, nextTimeColumnName)
                .ConfigureAwait(false);
        ...
        public virtual async Task<SchemeDefinition<XElement>> GetProcessSchemeWithParametersAsync(string schemeCode,
            Guid? rootSchemeId, bool ignoreObsolete, string tenantId = null)
        {
            IEnumerable<ProcessSchemeEntity> processSchemes;

            using (var connection = OpenConnection())
            {
                processSchemes = await WorkflowProcessScheme
                    .SelectAsync(connection, schemeCode, tenantId, ignoreObsolete ? false : (bool?)null, rootSchemeId)
                    .ConfigureAwait(false);
            }
...
#pragma warning restore 1998
        ...
        {
            using var connection = OpenConnection();
            var historyItem = (await WorkflowApprovalHistory.SelectByProcessIdAsync(connection, approvalHistoryItem.ProcessId)
                    .ConfigureAwait(false))
        ...
        public virtual async Task DropEmptyApprovalHistoryAsync(Guid processId)
        {
            ...
            foreach (var record in emptyHistoryItems)
            {
        ...
        {
            using var connection = OpenConnection();
            WorkflowFormEntity entity =
                await WorkflowForm.CreateNewFormIfNotExistsAsync(connection, _runtime.RuntimeDateTimeNow, name, defaultDefinition,
